...
export class SaveContactUseCase {
  ...
  async execute(dto: SaveContactDTO): Promise<{ contact: Contact; details: Individual | Company }> {
    ...
    if (isUpdate) {
      ...
      if (dto.email && dto.email !== existingContact.email) {
        const isDuplicate = await this.contactRepository.existsByEmail(existingContact.workspaceId, dto.email, existingContact.id);
        if (isDuplicate) throw new DuplicateEntityError('Контакт', 'Email', dto.email);
      }

      // Перевірка унікальності Телефону при оновленні (ігноруємо видалені)
      if (dto.phone && dto.phone !== existingContact.phone) {
        const isDuplicate = await this.contactRepository.existsByPhone(existingContact.workspaceId, dto.phone, existingContact.id);
        if (isDuplicate) throw new DuplicateEntityError('Контакт', 'номер телефону', dto.phone);
      }
      ...
      if (contact.contactType === 'individual') {
        const existingInd = await this.contactRepository.findIndividualByContactId(dto.id!);
        if (!existingInd) throw new EntityNotFoundError('Профіль фізичної особи', dto.id!);

        // Перевірка унікальності ІПН при оновленні (ігноруємо видалені)
        if (dto.taxNumber && dto.taxNumber !== existingInd.taxNumber) {
          const isDuplicate = await this.contactRepository.existsByTaxNumber(contact.workspaceId, dto.taxNumber, contact.id);
          if (isDuplicate) throw new DuplicateEntityError('Фізична особа', 'ІПН', dto.taxNumber);
        }
        ...
        const existingComp = await this.contactRepository.findCompanyByContactId(dto.id!);
        if (!existingComp) throw new EntityNotFoundError('Профіль компанії', dto.id!);

        // Перевірка унікальності ЄДРПОУ при оновленні (ігноруємо видалені)
        if (dto.taxId && dto.taxId !== existingComp.taxId) {
          const isDuplicate = await this.contactRepository.existsByTaxId(contact.workspaceId, dto.taxId, contact.id);
          if (isDuplicate) throw new DuplicateEntityError('Компанія', 'ЄДРПОУ', dto.taxId);
        }
      ...
      if (dto.contactType === 'individual') {
        if (!dto.firstName) throw new MissingRequiredFieldError('Ім’я');
        if (!dto.lastName) throw new MissingRequiredFieldError('Прізвище');
        
        if (dto.taxNumber) {
          const isDuplicate = await this.contactRepository.existsByTaxNumber(dto.workspaceId, dto.taxNumber);
          if (isDuplicate) throw new DuplicateEntityError('Фізична особа', 'ІПН', dto.taxNumber);
        }
      } else {
        if (!dto.companyName) throw new MissingRequiredFieldError('Назва компанії');

        if (dto.taxId) {
          const isDuplicate = await this.contactRepository.existsByTaxId(dto.workspaceId, dto.taxId);
          if (isDuplicate) throw new DuplicateEntityError('Компанія', 'ЄДРПОУ', dto.taxId);
        }
      }
      ...
      if (dto.email) {
        const isDuplicate = await this.contactRepository.existsByEmail(dto.workspaceId, dto.email);
        if (isDuplicate) throw new DuplicateEntityError('Контакт', 'Email', dto.email);
      }

      // Перевірка унікальності Телефону при створенні (ігноруємо видалені)
      if (dto.phone) {
        const isDuplicate = await this.contactRepository.existsByPhone(dto.workspaceId, dto.phone);
        if (isDuplicate) throw new DuplicateEntityError('Контакт', 'номер телефону', dto.phone);
      }
