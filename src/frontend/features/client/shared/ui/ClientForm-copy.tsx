...
export function ClientForm({
  ...
  const initialValues = useMemo<CreateClient>(() => {
    ...
    // Гарантуємо, що поля паспорта не null
    if (baseIndividual.passportDetails) {
      baseIndividual.passportDetails = {
        series: baseIndividual.passportDetails.series ?? "",
        number: baseIndividual.passportDetails.number ?? "",
        issuedBy: baseIndividual.passportDetails.issuedBy ?? "",
        issuedDate: baseIndividual.passportDetails.issuedDate ?? "",
      };
    } else {
      baseIndividual.passportDetails = {
        series: "",
  ...
  // Оновлюємо форму при зміні defaultValues
  useEffect(() => {
    if (defaultValues) {
      reset(initialValues);
    }
